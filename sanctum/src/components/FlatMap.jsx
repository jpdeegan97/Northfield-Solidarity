                ...
                {data.children && data.children.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {data.children.map((childId) => {
                            ...
                            // Deterministic random-like values based on childId
                            const pseudoRandom = childId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                            const isActive = pseudoRandom % 5 !== 0; // 80% chance true
                            const progress = (pseudoRandom % 100);
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        ...
                                        {/* Mock Status Dot */}
                                        <div className={`w-1.5 h-1.5 rounded-full ${!isActive ? 'bg-yellow-500' : 'bg-emerald-500'}`} />
                                    </div>
                                    <div className="text-[9px] text-white/40 font-mono uppercase">{childData.type || 'REGION'}</div>

                                    {/* Mock Mini-Vis */}
                                    <div className="w-full h-1 bg-white/10 rounded-full mt-2 overflow-hidden">
                                        <div className="h-full bg-white/30 group-hover:bg-brand transition-colors" style={{ width: `${progress}%` }} />
                                    </div>
                                </button>
                            );
                        })}
