...
export default function ManifoldView({ engine }) {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setTraceLog(prev => [`[${timestamp}] ${newAction}`, ...prev].slice(0, 15));
                    ...
                    <div className="flex-1 bg-black rounded-xl border border-white/10 p-4 font-mono text-xs overflow-hidden relative">
                        ...
                        <div className="mt-8 flex flex-col gap-1 h-full overflow-y-auto pb-4 custom-scrollbar">
                            <AnimatePresence initial={false}>
                                {traceLog.map((log, i) => (
                                    <motion.div
                                        key={i} // Using index as key for simple rolling log
                                        initial={{ opacity: 0, x: -10 }}
                                        animate={{ opacity: 1, x: 0 }}
                                        className="text-white/70 border-l border-white/10 pl-3 py-1 hover:bg-white/5 hover:text-white transition-colors cursor-default"
                                    >
                                        <span className="text-emerald-500/50 mr-2">{log.split(']')[0]}]</span>
...
// Add simple scrollbar styles if not present globally
