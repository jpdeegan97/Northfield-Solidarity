...
- [ ] **Audit "Skeleton" Engines:** Review the following lightweight engines (currently <2KB) to ensure they have at least MVP functionality:
  - \`FRK\` (Franklin)
  - \`INC\` (Incorporator)
  - \`CRN\` (Chronos)
- [ ] **Standardize UI/UX:** Ensure all 25+ engines share consistent headers, window controls, and design tokens (colors/fonts).
- [ ] **Mock Data Robustness:** Replace any remaining "Lorem Ipsum" or hardcoded placeholders with dynamic-looking mock data generators (like we did for \`WPV\`).
...
- [ ] **Verify Checks:** Verify \`WPV\` (White Paper Visualizer) is accessible from *all* context menus where documentation exists (completed for Marketplace/Docs, check others).
