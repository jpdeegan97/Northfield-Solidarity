...
export const ROAM_CHARTER = `NS-ROAM-000 — Project Charter
Project: NS ROAM (Mobile DevOps Cockpit)
Version: 1.1
Date: 2025-12-30

## Mission
Enable uninterrupted full-stack development and operations from anywhere, with the phone as a secure, first-class command-and-control surface and reproducible remote workspaces doing the heavy compute.

## Product Thesis
NS ROAM packages a cohesive workflow: phone cockpit + action broker + policy gates + preview envs + audit + mobile dashboards.

## MVP Scope (Status: Active Build)
- Overlay access guidance and templates
- roam CLI bootstrap scripts
- GitHub Actions templates (phone-triggerable)
- Minimal “Phone Dashboard” spec (LUM integration)
- 5 phone-first runbooks`;

export const WPV_CHARTER = `NS-WPV-000 — CHARTER
...
export const WISHLIST_CHARTER = `NS-WISHLIST — CHARTER

## Mission
Build the best “buying decision copilot” on the internet: you enter what you want to buy, ** WISHLIST ** compiles research + availability + alternatives, and produces a ranked set of purchase options based on your priorities(fastest, cheapest, best value, local pickup, trusted seller).

## Product thesis
The modern buying process is broken by fragmentation:
- listings vary wildly(SKUs, variants, bundles, refurbs, knockoffs),
  - availability / ETAs shift constantly,
    - “best deal” depends on hidden variables(shipping, returns, warranty, seller trust),
      - research is repetitive and time - expensive.

WISHLIST compresses this into a single, explainable decision surface.

## Core promises
  - ** Canonical truth ** for what the product is(including variant identity)
    - ** Rankings with explanations ** (not just sorting)
- ** Provenance + freshness ** for every fact that matters
  - ** Alerts ** that remove re - checking

## Non - promises(explicit boundaries)
  - Not merchant of record; we do not process payments(MVP)
    - We do not guarantee inventory correctness; we report confidence + timestamps
      - We do not scrape in ways that violate source terms; preference for APIs / feeds / partners

## Ethics and trust policy
  - Never hide a better option because of affiliate payout differences
    - Sponsored content must be explicit and separated from “best” rankings
      - User - first defaults: conservative matching, clear confidence, minimal data retention

## Success metrics
  - Time - to - decision
  - Match correctness for top sources
    - Alert usefulness
    - Conversion(click - through to selected purchase option)
  - Trust(low dispute / complaint rate)

## Fit inside Northfield Solidarity
  - DRE: research synthesis + monitoring
    - Switchboard: model / tool routing for extraction / verification
      - Quarentine: cleansing / normalization
        - LUM: observability + product analytics
          - GGE: governance of sources, disclosures, and policies
