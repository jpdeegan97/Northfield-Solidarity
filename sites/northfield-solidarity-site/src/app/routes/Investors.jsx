...
import PartnerRoyaltyView from '../../components/PartnerRoyaltyView.jsx';
import RevenueWhitePaper from '../../components/RevenueWhitePaper.jsx';
...
import { FinancialModel } from '../../logic/FinancialModel.js';
...
export default function Investors() {
    ...
    const { projects, pledges, leaderboard, makePledge } = useCrowdFunding();
                ...
                <div className="investor-portal">
                    ...
                    {pledgeModalOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                            <div className="bg-slate-900 border border-emerald-500/50 p-8 rounded-xl max-w-md w-full shadow-2xl relative overflow-hidden">
                                {pledgeSuccess ? (
                                    <div className="text-center py-8 animate-in zoom-in duration-300">
                                        <div className="text-6xl mb-4">🎉</div>
                                        <h3 className="text-2xl font-bold text-emerald-400 mb-2">Pledge Confirmed!</h3>
                                        <p className="text-slate-300">You've successfully backed {selectedProject?.name}.</p>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <form onSubmit={submitPledge}>
                                            <div className="mb-6">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-black/50 border border-slate-700 rounded p-4 text-2xl text-white focus:border-emerald-500 focus:outline-none font-mono mb-3"
                                                />
                                                <div className="flex gap-2">
                                                    {[100, 500, 1000, 5000].map(amt => (
                                                        <button
                                                            ...
                                                            className={`flex-1 py-1 text-xs rounded border transition-all ${parseInt(pledgeAmount) === amt
                    ...
                    {activeTab === 'projections' && (
                        <div className="tab-content fade-in">
                            <section className="ir-section">
                                <h3 className="section-label">Projected Operational Metrics (Year 1 Target)</h3>
                                {(() => {
                                    // Calculate Year 1 Projections (Base Case)
                                    const model = new FinancialModel('BASE').runProjection();
                                    const y1 = model.timeline[11].metrics; // Month 12
                                    const prevMonth = model.timeline[10].metrics;

                                    const arr = (y1.arr / 1000).toFixed(0);
                                    const mrrGrowth = ((y1.mrr - prevMonth.mrr) / prevMonth.mrr * 100).toFixed(1);

                                    return (
                                        <div className="metrics-grid">
                                            <div className="metric-card">
                                                <div className="metric-label">Projected ARR (Y1)</div>
                                                <div className="metric-value text-emerald-400">${arr}k</div>
                                                <div className="metric-delta positive">Based on {model.scenario}</div>
                                            </div>
                                            <div className="metric-card">
                                                <div className="metric-label">Active Seats (Y1)</div>
                                                <div className="metric-value">{y1.activeUsers}</div>
                                                <div className="metric-delta positive">+{mrrGrowth}% MoM</div>
                                            </div>
                                            <div className="metric-card">
                                                <div className="metric-label">Gross Efficiency</div>
                                                <div className="metric-value">82%</div>
                                                <div className="metric-delta neutral">SaaS Std.</div>
                                            </div>
                                            <div className="metric-card">
                                                <div className="metric-label">Service Revenue</div>
                                                <div className="metric-value">${(y1.serviceRevenue / 1000).toFixed(1)}k</div>
                                                <div className="metric-delta neutral">/ month</div>
                                            </div>
                                        </div>
                                    );
                                })()}
                    ...
                    {activeTab === 'funding' && (
                        <div className="tab-content fade-in">
                            <section className="ir-section">
                                ...
                                {pledges.length > 0 && (
                                    <div className="mb-8 overflow-hidden relative bg-slate-900/50 border-y border-slate-800 py-3">
                                        <div className="flex gap-8 animate-scroll whitespace-nowrap">
                                            {[...pledges, ...pledges].map((p, idx) => (
                                                <div key={idx} className="flex items-center gap-2 text-sm text-slate-400">
                                                    <span className="text-emerald-400 font-bold">+${p.amount}</span>
                                                    <span>from {p.userName}</span>
                                                    <span className="text-slate-600">to {p.projectCode}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
