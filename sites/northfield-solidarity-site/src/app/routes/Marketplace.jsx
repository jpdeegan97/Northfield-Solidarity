...
import { ShoppingCart, X, Trash2, CreditCard, ChevronRight, Scale, Search, Settings } from 'lucide-react';
import AssetDetailOverlay from './marketplace/AssetDetailOverlay';
...
// WSP Mock
const WSP_ASSETS = [
    { id: 'wsp1', name: 'High-Freq Arb Bot', category: 'ALGORITHMS', group: 'ALGORITHMS', basePrice: 2500000, tier: 'HYPER', desc: 'Sub-ms latency exchange arbitrage.' },
    { id: 'wsp2', name: 'Macro Volatility Index', category: 'DATA', group: 'ALGORITHMS', basePrice: 175000, tier: 'TUNED', desc: 'Aggregated cross-asset fear gauge.' },
    { id: 'wsp3', name: 'Options Pricing Engine', category: 'ANALYTICS', group: 'ALGORITHMS', basePrice: 750000, tier: 'PERFORMANCE', desc: 'Black-Scholes-Merton on steroids.' },
    { id: 'wsp4', name: 'Liquidity Hunter SDK', category: 'SDK', group: 'ALGORITHMS', basePrice: 500000, tier: 'PERFORMANCE', desc: 'Detects hidden liquidity pools across exchanges.' },
    { id: 'wsp5', name: 'Sentiment Alpha API', category: 'API', group: 'ALGORITHMS', basePrice: 125000, tier: 'TUNED', desc: 'Real-time NLP sentiment analysis from news feeds.' },
    { id: 'wsp6', name: 'Risk Parity Solver', category: 'SDK', group: 'ALGORITHMS', basePrice: 300000, tier: 'STOCK', desc: 'Automated portfolio rebalancing logic.' },
    { id: 'wsp7', name: 'Yield Farmer Core', category: 'ALGORITHMS', group: 'ALGORITHMS', basePrice: 900000, tier: 'PERFORMANCE', desc: 'Cross-chain yield optimization strategy.' },
];
...
export default function Marketplace() {
    ...
    // --- GROUP CONFIGURATION ---
    const GROUPS = ['ALL', 'ENGINES', 'PROJECTS', 'ALGORITHMS', 'API INTEGRATIONS', 'MISC'];
    ...
    const getFinalPrice = (asset) => {
        if (asset.finalPrice) return asset.finalPrice;
        return Math.floor(asset.basePrice * scaleConfig.val);
    };

    const addToCart = (asset) => {
        // Generate a Cart ID if one doesn't exist (from Overlay it might, or we generate here to allow multiples)
        const itemToAdd = { ...asset, cartId: asset.cartId || `${asset.id}-${Date.now()}` };

        // Optional: Check for duplicates based on original ID if we want to limit 1 per type?
        // For now let's allow multiples or just rely on user.
        // If we strictly want to block same ID:
    ...
    const cartTotal = cart.reduce((sum, item) => sum + getFinalPrice(item), 0);
                                        ...
                                        cart.map(item => (
                                            <div key={item.id} className="bg-white/5 border border-white/5 rounded p-3 flex gap-3 group relative">
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    ...
                                                    <div className="text-xs font-mono mt-1" style={{ color: accentColor }}>${getFinalPrice(item).toLocaleString()} USD</div>
                                                    {item.selectedOptions && (
                                                        <div className="flex flex-wrap gap-1 mt-2">
                                                            {item.selectedOptions.map(opt => (
                                                                <span key={opt.id} className="text-[9px] px-1 rounded bg-white/10 text-white/60">{opt.title}</span>
                                                            ))}
                                                        </div>
                                                    )}
                                                </div>
        ...
        >
            ...
            >
                <span className="text-black font-bold text-xs tracking-widest flex items-center gap-1">
                    <Settings size={12} className="text-black" /> CONFIGURE
                </span>
            </div>
        </motion.div>
    );
